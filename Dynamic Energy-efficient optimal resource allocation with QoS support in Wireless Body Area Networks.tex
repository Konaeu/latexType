...
\title{Data-Rate-Adaption Assisted and Energy-efficient Resource Allocation with QoS Support in WBANs}
...
With the advances in wearable sensors and wireless technologies, wireless body area network (WBAN) has become a promising network to provide health applications such as ubiquitous e-Health services and real-time health monitoring. In these WBANs, Quality of Service (QoS) requirements, including packet loss rate (PLR), throughput and delay, needs to be guaranteed for providing reliable and real-time data transmission even under the highly dynamic environment due to human mobility and postures. Meanwhile, energy efficiency is another important issue to be ensured to increase the network lifetime in such a resource-constrained network. In this paper, a data-rate-adaption assisted and energy-efficient resource allocation scheme is proposed in which both constraints of QoS metrics and the characteristics of dynamic links are considered. Specifically, a QoS optimization problem is formulated to optimize the transmission power and allocated time slots for each sensor, which minimizes energy consumption subject to the QoS constraints. Due to the dynamic link characteristics, even the maximum transmission power cannot satisfy the QoS requirements with some bad link quality. To guarantee the QoS requirements when the link is very poor, a data rate adaption policy (DRAP) is proposed to carefully adjust the data rate at each sensor to assist the QoS optimization problem. Numerical results demonstrate the effectiveness of the proposed data rate allocation policy and the QoS optimization problem.
\end{abstract}
\textbf{Keywords:}
Wireless body area network (WBAN), quality of service (QoS), energy efficiency, data rate adaption policy, QoS resource allocation 
...
With the rapid increase of the aging population, health care costs for the elderly is rising and will overload health care systems\cite{myReference:WBAN:survey}. Therefore, wireless body area network (WBAN), which contains one hub (PDA or Mobile Phone) and body area sensor nodes, has emerged as a key technology to provide applications with high healthcare efficiency. Since WBAN based on high energy-efficient technologies can satisfy many applications in Electronic Health(e-Health) services, e.g., remote patient monitoring and medical care \cite{myReference:Survey:WBAN:MAC,myReference:Survey:WBAN:2}, healthcare community and engineering industry have shown increased interests in WBAN.

For these health applications, physiological data streams should be collected and transmitted reliably from humans via WBAN. A loss or an excessive delay of the vital physiological data streams acquired by body sensors may cause a fatal accident \cite{myReference:Optimal:Resource}. For example, heart activity readings(e.g. ECG readings) should be monitored continuously for the detection of a heart attack. Once the heart attack is detected, a warning signal will be sent to the health professionals for the timely assistance. Hence, a loss of heart activity readings may cause a fatal accident, and even lead to the death of the patient. Therefore, guaranteeing the quality of service (QoS) of WBAN, such as packet loss rate (PLR), throughput, and delay, becomes an important issue, especially when the link quality is dynamic due to human mobility and postures \cite{myReference:WBAN:survey2,myReference:QoS:BodyQos2}.

QoS issue of WBAN is different from that of Internet, ad hoc networks and wireless network due to the special characteristics of WBAN: (1) A wireless body area network system is a heterogeneous and resource-constrained system, which consists of several simple sensor nodes and a powerful hub. Because of the requirement of lightweight and non-instrusive, the size of body sensors is limited and the resource such as processing, storage and battery energy supply are extremely constrained comparing with ordinary wireless sensors. Additionally, replacing the batteries will cause the interruption of health monitoring, which is not expected and also may be fatal while the critical data are not captured in time. (2) Link characteristics are dynamic and should be taken seriously. Channel fading between the transmitter and receiver is subject to the link distance and a number of factors such as clothing, obstructions due to different body segments and so on \cite{myReference:Posture:BodyPosture}. When the posture changes, some factors of the link will inevitably change. Therefore, on-body sensor networks have to deal with such link dynamism caused by the changing of the posture. Furthermore, the link quality may be so bad that QoS requirements are hard to be guaranteed when the environment changes or the posture changes\cite{myReference:Channel:Characterization}. Obviously, these characteristics and QoS issue are tightly interdependent and mutually influenced. Thus how to overcome the QoS issue while energy-efficient characteristic and link-dynamic characteristic are both carefully considered is a key challenge in WBAN. 

In this paper, we investigate all these issues and explore all useful resources to try best to satisfy the QoS requirements. A data rate adaption assisted resource allocation solution is formulated, which minimizes energy consumption subject to the constraints of QoS metrics, such as packet loss rate (PLR), delay and throughput, while characteristics of dynamic links with different postures are considered. The key contributions of this paper are two-fold:

First, we propose a QoS optimization problem to minimize energy consumption, subject to constraints of QoS metrics, such as PLR, delay and throughput. The original nonlinear and non-convex optimization problem is successfully transformed to the form of Generalized Geometric Programming (GGP) which can be solved efficiently. We also take the characteristics of dynamic links with different postures into consideration and reallocate resources for each sensor by resolving the optimization problem when the posture changes. 

Secondly, we introduce a data rate adaption policy (DRAP) before the QoS optimization problem to achieve more strict PLR constraint by adjusting transmission rate. When link quality is very poor, adopting fixed transmission rate cannot guarantee stricter PLR constraint, even if transmission power is set to the maximum. However, with the same signal-to-noise ratio (SNR), reducing transmission rate can gain lower PLR. Therefore, we develop the TRAP that dynamically adjusts transmission rate based on link quality and PLR requirement. TRAP is easy to implement, and can be tuned for desired trade-off between energy consumption and attainable PLR with stricter PLR constraint.
...
To combat the aforementioned QoS issue with considering energy and dynamic link characteristics to improve WBAN performance, many strategies have been provided in the literature. Transmission power control as a classic approach of reducing communication cost has been studied to improve energy efficiency of wireless network in \cite{myReference:TPC:Prediction,myReference:TPC:TPCandLA,myReference:TPC:Healthcare,myReference:Posture:BodyPosture,myReference:TPC:LSETPC}. The authors in \cite{myReference:TPC:Healthcare} outlined that the wireless link quality can change rapidly using the empirical evidence and fixed transmit power would result in wasted energy or low reliability. Thus a power control scheme was proposed to adjust transmit power based on feedback from the receiver and the parameters of the scheme can be tuned to achieve different trade-offs between energy savings and reliability. However,  only reliability as one QoS metric was taken into  consideration. In \cite{myReference:TPC:Prediction}, the authors presented a novel channel prediction scheme utilizing the partial-periodicity of measured BAN channels. However, complex and rapid changes of the body postures seriously affect the accuracy of the channel prediction. In \cite{myReference:Posture:BodyPosture}, the dynamic nature of on-body links with varying body postures was characterized, then a dynamic postural position inference (DPPI) mechanism was proposed to assign the best possible power level to a link, which was based on the observed linear relationship between transmission power (TP) and received signal strength indicator (RSSI) \cite{myReference:TPC:ATPC}. However, the linear relationship between TP and RSSI was studied in wireless sensor networks, in which the wireless sensors were usually located in a fixed position. Thus it was inappropriate to directly adopt it in WBAN case.
...
A typical WBAN model as shown in Fig.~\ref{WBAN}, which contains one hub and body sensor nodes deployed on the different positions of human body. The set of body sensors is defined as $\mathcal{N}$. And these sensors collect physiological signals and transmit them to the hub for further processing. At the network layer, one hop star topology is adopted with considering that resources of body sensor are extremely limited and additional relay nodes can bring uncomfortable user experience, while star and tree topologies are both recommended in IEEE 802.15.6 standards \cite{myReference:WBAN:Standard}. At the data-link layer, collision-free and TDMA-based scheduled access mechanism in beacon mode with superframe boundaries is adopted. As presented in Fig.~\ref{TDMA}, the hub broadcasts beacons to define the superframe boundaries and allocates the slots to all the sensors, and the sensor node turns active in its dedicated slots to transmit data, which greatly reduce the energy consumption. In addition, dedicated slot allocation naturally avoid collisions, idle listening, and overhearing of sensor nodes. At the physical layer, the Industrial, Scientific, and Medical (ISM) band is adopted, which can support high-data-rate applications. Differential Phase Shift Keying (DPSK) modulation is used in ISM band \cite{myReference:WBAN:Standard}. And several level information data rates $\Re = \{ Rat{e_1},\cdots,Rat{e_h}\}$ are optional through configuring the parameters related to the Bose-Chaudhuri-Hocquenghem(BCH) code rate and modulation order.
...
In WBANs, energy consumption of a body sensor mainly consists of two parts: circuitry energy consumption ${E_{elec}}$ and transmit amplifier energy consumption ${E_{tx}}$, while energy consumption of processing and listening can be negligible \cite{myReference:Energy:Human, myReference:TPC:Healthcare}. And we further assume that total transceiver energy consumption $E_{con}$ is proportional to transmit amplifier energy consumption ${E_{tx}}$ \cite{myReference:Energy:Energymode}. Therefore, the formula of the energy model is shown as follows,
...
To satisfy the QoS requirement, the transmission power is adjusted based on the path loss of the wireless link. Therefore, transmit amplifier energy consumption is dependent on the path loss model in wireless body area networks. Recommended by the IEEE 802.15.6 \cite{myReference:WBAN:Standard}, the path loss can be modeled as the following equation:

\begin{equation}\label{PL}
PL\left( d_{i} \right) ={PL_{i,d_0,dB}} + 10n{\log _{10}}\left( {\frac{d_{i}}{{{d_0}}}} \right) + {X_\sigma }
\end{equation}\\
where $d_{i}$ is the distance between sensor $i$ and the hub. ${PL_{i,d_0,dB}}$ is the path loss at a reference distance ${d_0}$ at sensor $i$, and $n$ is the path-loss exponent which is different between with Line-Of-Sight(LOS) channel and Not-Line-Of-Sight(NLOS) channel. The shadowing ${X_\sigma }$ in $dB$ follows a normal distribution $N\left( {0,\sigma^2 } \right)$. When the body postures changes, such as from still to run, the average and the standard deviation of shadowing ${X_\sigma }$ will be varies correspondingly with the changing factors of the link\cite{myReference:Channel:Statistical}. Therefore, body postures, which have the great impact on the characterization of the wireless channel, need be carefully taken into account when we design the resource optimization problem. As for the posture detection, many methods were proposed to classify the posture in real time with the physiological signals and quit a few extra motion sensors by the resource-rich Hub\cite{myReference:Posture:Real-time,myReference:Posture:PostureAndMovement}.Therefore, in this paper, we assume the posture can be detected in real time by the resource-rich hub while the posture changes.

\subsection{Queuing Model}
Each wireless sensor node can collect the physiological signals and packetize them in packets. Obviously one of the main distinguishing characteristics of WBANs is that WBANs may contain critical physiological signals, which would be picked out and treated with stricter QoS requirements. Therefore, each sensor node has a packet classifier module which can classify the packets into one of the two classes: normal packets and emergency packets, based on the preset thresholds at each sensor\cite{myReference:MAC:TrafficAdaptive}. Correspondingly, these normal packets and emergency packets are put into the normal queue and the emergency queue. Here, normal packets are generated on periodic basis, while emergency packets are totally non-periodic. Packets in different queues are treated respectively with different QoS requirements. For example, the delay of the emergency packets is shorter than the delay of the normal packets.

For normal packets, we assume the periodic arrivals of the normal packets at node $i$ are a constant ${\lambda _{i,n}}$ during a superframe. And the normal packet transmission process can be modeled as a $D/G/1$  queuing model, and the service rate $\mu_{i,n}$ follows a Binomial Distribution in consideration of the existence of the packet loss. The upper boundary of average queue delay ${W_{q,i,n}}$ of the $D/G/1$ queuing model can be expressed as follows

\begin{equation}
W_{q,i,n} \le \dfrac{\lambda_{i,n} {\sigma _{i,n,B}^2}}{2\left({1 - \rho_{i,n} }\right)}T
\end{equation}\\
where $\rho_{i,n} = \frac{\lambda_{i,n} }{\mu_{i,n}}$, $\mu_{i,n}  = \frac{{\left({1 - PLR_{i,n}} \right)R_{i,n}t_{i,n}}}{L_{i,n}}$, and $\sigma _{i,n,B}^2 = \frac{{PLR_{i,n} \left( {1-PLR_{i,n}}\right)R_{i,n}t_{i,n}}}{L_{i,n}}$. Here $R_{i,n}$ is the data rate of transmitting the packet to the hub for normal packets at sensor $i$, $t_{i,n}$ is the length of scheduled time at sensor $i$ for normal packets, and $PLR_{i,n}$ is the packet loss rate of normal packets at sensor $i$. $L _{i,n}$ is the length of the normal packet in bits at node $i$.

For emergency packets, we assume the arrivals of the emergency packets at node $i$ follow a Poisson process with an average rate $\lambda _{i,m}$ during a superframe. And the service rate $\mu_{i,m}$ follows a Binomial Distribution due to the packet loss at sensor $i$. Thus we can model the emergency packet transmission process as a $M/G/1$ queuing model. The average queuing delay of the emergency packet is given by \cite{myReference:Book:Queuing}:
...
We consider three QoS metrics, the packet loss rate (PLR) and the delay, of the normal and emergency packets respectively.
...
In the ISM band, the raw data firstly need to be coded using channel coding, e.g. BCH code, which can improve the performance of the fading channel. Then the coded data go through the modulator to be further transmitted to the hub \cite{myReference:WBAN:Standard}. Therefore, with the assumption of the bit errors occur independently in a packet, the packet loss rate $PLR$ can be expressed as follows, 

\begin{equation}\label{PLR}
PLR({\gamma }) = 1 - {\left( {1 - {P_{b,C}}({\gamma })} \right)^L}
\end{equation}\\
where ${\gamma } =  {10^{\frac{{{P_{tx,dB}} - PL\left( d \right) - {P_{N}}}}{{10}}}}   \frac{B}{R}
$ is the bit Signal to Noise Ratio(bit SNR). ${P_{tx,dB}}$ indicates the transmission power in $ dB$, ${P_{N}}$ is the power of noise in $ dB$, $B$ is the system bandwidth, $R$ is the transmission rate.  $P_{b,C}$ is the bit error rate performance error using channel coding and is a monotonous decreasing function of bit SNR $\gamma$. $L$ is the length of the packet in bits. 

Due to the time-varying channel, the path loss in the current superframe cannot be known in advance. Especially, when body postures change, the path loss cannot even be estimated appropriately based on the history of the link. However, the standard deviation of shadowing can be obtained through experiments \cite{myReference:Channel:Statistical}, and the average of SNR can also be adjusted by adjusting the transmission power. Then we calculate average PLR for different postures as follows,
...
%For both normal packtes and emergency packets, the queuing system needs to satisfy the throughput condition, i.e., $\mu  \ge \lambda $, in order to be stable \cite{myReference:Book:Queuing}. For normal packets, the throughput constraint $\mu _{i,n} \ge \lambda _{i,n}$ at sensor $i$ can be expressed as,
%
...
The total delay that a packet suffers contains three parts: access delay ${W_a}$, propagation delay ${W_p}$ and queuing delay ${W_q}$, where access delay ${W_a}$ is the time between the packet arrival and the beginning of the scheduled slots in the next superframe, propagation delay ${W_p}$ is the time from the begin transmission to the end of a packet, and queuing delay ${W_q}$ is the average time that the packet waits in the queue until it is successfully transmitted, shown in Queuing Model. The average access delay is half of the superframe time $T/2$ when we assume the packets arrive uniformly during each superframe time. Propagation delay is related to the service rate in the queuing system, which can be expressed as ${W_p = t/\mu}$. Therefore, the delay constraint can be formulated as,

\begin{equation}
W = {W_a}{\text{ + }}{W_q}{\text{ + }}{W_p}  \le {D_{th}}
\end{equation}\\

As for the delay constraint threshold ${D_{th}}$, that of emergency packets is smaller than that of normal packets for more higher priority. 

\section{QoS Optimization Problem}
\subsection{QoS Optimization Problem Statement}
We optimize the resource allocation in WBAN to guarantee QoS requirements, meanwhile the energy efficiency of WBAN system is fully considered. The QoS optimization problem can be stated as: to minimize sum energy consumption of all the body sensors for both normal packets and emergency packets, subject to constraints of QoS metrics, such as PLR, throughput and delay, which are given in section ~\ref{QoSConstraint}. For different postures, channel fading parameters are different, therefore, we have to resolve the QoS problem while the posture is detected and changing. Mathematically, the problem is formulated as follows,
% GGP
...
&&&{P_{tx,\min}} \le P_{tx,i,n} \le {P_{tx,\max}},&\\
&&&{P_{tx,\min}} \le P_{tx,i,m} \le {P_{tx,\max}},&
\end{align*}
\end{gather} \\
where $\bm{P_{p}}$ is the vector of the transmission powers ${P_{tx,i,n}},{P_{tx,i,m}},\left(i=1,..,N\right)$ for normal packets and emergency packets of all sensor nodes at posture state $p$, where $p\in \{pos_{1},...,pos_{k}\}$. $\bm{T_{p}}$ is the vector of the allocated time ${t_{i,n}} ,{t_{i,m}},\left(i=1,..,N\right)$ for normal packets and emergency packets of all sensor nodes at posture state $p$.  $PL{R_{i,n,th}},{PL{R_{i,m,th}}}$ are the thresholds of the average PLR for normal packets and emergency packets at sensor $i$ respectively.  ${D_{i,n,th}},{D_{i,m,th}}$ are the thresholds of the delay for normal packets and emergency packets at sensor $i$ respectively. ${P_{tx,\min}},{P_{tx,\max}}$ are the minimum and maximum transmission power respectively. 

In above Qos optimization problem, the objective is to minimize the sum of the energy consumption of all wireless sensor nodes for normal packets and emergency packets in a superframe. And the first and second constraints are PLR constraints for normal packets and emergency packets respectively, in the form of eq~\eqref{eqPLR}, and require that average PLR of normal and emergency packets should be less than predefined PLR thresholds. And the PLR threshold of emergency packets is generally smaller than normal packets. The third and fourth constraint are throughput constraints, which require that service rates $\mu$ should be larger than arrival rates $\lambda$ to satisfy the stable condition of the queuing system. The fifth and sixth constraints are delay constraint and require that average delay of normal and emergency packets should be smaller than the delay threshold. 

\subsection{Optimal Numerical Solution}
Above optimization problem is a nonlinear and non-convex problem, and it is difficult to be solved. Fortunately, we try best to transform the original problem to the form of Generalized Geometric Programming (GGP), which can be solved reliably and efficiently \cite{myReference:GGP:YALMIP}.

\begin{thm}
The average PLR \eqref{eqPLR} is a monotonically decreasing function of ${\mu _{{\gamma _{dB}}}}$.
\end{thm}
\begin{IEEEproof}
Obviously, the packet loss rate ${PLR({\gamma })}$ is a monotonous decreasing function of bit SNR ${\gamma}$. Once the posture has been decided, the standard deviation ${\sigma _{{\gamma _{dB}}}}$ is determinate.
...
$\overline {PLR\left( {{\mu _{{\gamma _{dB}}}},{\sigma _{{\gamma _{dB}}}}} \right)} \\		 
     \text{     }\text{     }= {1 - }\int\limits_0^{ + \infty } {PLR({\gamma })} \frac{{10/\ln 10}}{{\sqrt {2\pi } {\sigma _{{\gamma _{dB}}}}{\gamma }}}\exp \left[ { - \frac{{{{(10lo{g_{10}}({\gamma }) - {\mu _{{\gamma _{dB}}}})}^2}}}{{2\sigma _{{\gamma _{dB}}}^2}}} \right]d{\gamma }\\
		\text{     }\text{     }=  1- h\left( {{\mu _{{\gamma}}}} \right)	$

The equivalent proof is to prove the $h\left( {{\mu _{{\gamma}}}} \right)$ is a monotonous increasing function of the average of bit SNR $\mu _{{\gamma}}$. Therefore, if  
$\forall {\text{ }}{\mu _{{\gamma _{1}}}},{\mu _{{\gamma _{2}}}}$, ${\mu _{{\gamma _{1}}}} \ge {\mu _{{\gamma _{2}}}}$,
and we set ${x_{\text{1}}} = \frac{{{\gamma }}}{{{\mu _{{\gamma _{1}}}}}},{x_{\text{2}}} = \frac{{{\gamma }}}{{{\mu _{{\gamma _{2}}}}}}$. So we need to prove that $h\left( {{\mu _{{\gamma _1}}}} \right) \ge h\left( {{\mu _{{\gamma _2}}}} \right) $ is permanent establishment:\\
${h\left( {{\mu _{{\gamma _1}}}} \right) = \int\limits_0^{ + \infty } {PLR({\mu _{{\gamma _1}}}{x_{\text{1}}})} \frac{{10/\ln 10}}{{\sqrt {2\pi } {\sigma _{{\gamma _{dB}}}}{x_{\text{1}}}}}\exp \left[ { - \frac{{{{\left( {10{{\log }_{10}}{x_{\text{1}}}} \right)}^2}}}{{2\sigma _{{\gamma _{dB}}}^2}}} \right]d{x_{\text{1}}}}$
${h\left( {{\mu _{{\gamma _2}}}} \right) = \int\limits_0^{ + \infty } {PLR({\mu _{{\gamma _2}}}{x_{\text{2}}})} \frac{{10/\ln 10}}{{\sqrt {2\pi } {\sigma _{{\gamma _{dB}}}}{x_{\text{2}}}}}\exp \left[ { - \frac{{{{\left( {10{{\log }_{10}}{x_{\text{2}}}} \right)}^2}}}{{2\sigma _{{\gamma _{dB}}}^2}}} \right]d{x_{\text{2}}}}$
Then,\\
 $h\left( {{\mu _{{\gamma _1}}}} \right) - h\left( {{\mu _{{\gamma _2}}}} \right)\\
  = \int\limits_0^{ + \infty } {PLR_{\Delta}\left(x\right)} \frac{{10/\ln 10}}{{\sqrt {2\pi } {\sigma _{{\gamma _{dB}}}}x}}\exp \left[ { - \frac{{{{\left( {10{{\log }_{10}}\left( x \right)} \right)}^2}}}{{2\sigma _{{\gamma _{dB}}}^2}}} \right]dx$\\
where $PLR_{\Delta}\left(x\right)={PLR({\mu _{{\gamma _1}}} x) - PLR({\mu _{{\gamma _2}}} x)}$. Because the ${PLR({\gamma })}$ is a monotonous decreasing function of bit SNR ${\gamma}$, and ${\mu _{{\gamma _{1}}}x} \ge {\mu _{{\gamma _{2}}}x}$, therefore, $PLR_{\Delta}\left(x\right) \ge 0$. So we prove that $h\left( {{\mu _{{\gamma}}}} \right)$ is a monotonous increasing function of the average of bit SNR $\mu _{{\gamma}}$.

\end{IEEEproof}

Since the average PLR in eq~\eqref{eqPLR} decreases with the increase of ${\mu _{{\gamma _{dB}}}}$. So the threshold $\mu _{th}$ of ${\mu _{{\gamma _{dB}}}}$ is obtained with which the average PLR just meets the PLR constraint \eqref{eqPLR}, while ${\sigma _{{\gamma _{dB}}}}$ is decided in line with the current posture. Therefore the average packet loss rate constraint \eqref{eqPLR} can be transformed into the ${\mu _{{\gamma _{dB}}}}$ constraint expressed as 

\begin{equation}\label{muPLR}
\begin{split}
{\mu _{{\gamma _{dB}}}} & = E\left[ {10{{\log }_{10}}\left( {\frac{B}{R}} \right) + {P_{tx,dB}} - {PL\left( d \right)}- {P_{N}}} \right]\\
& = 10{\log _{10}}\left[ {{P_{tx}}   \frac{B}{R}} \right] -PL_{Fr}\left( {d}\right) - {P_{N}}\\
& \ge \mu _{th}
\end{split}
\end{equation}
where $\mu _{th}$ is the threshold of ${\mu _{{\gamma _{dB}}}}$, which just satisfy $\overline {PLR\left( {{\mu _{{th}}},{\sigma _{{\gamma _{dB}}}}} \right)}=PLR_{th}$.

Therefore, the first and second PLR constraints, in form of eq~\eqref{muPLR}, can be transformed into the form of generalized posynomials:
\begin{equation}\label{FinalEqPLR}
\begin{split}
P_{tx}{R^{{ - 1}}} & \ge {B^{ - 1}} {10^{\frac{{\mu _{th}+ PL_{Fr}\left({d}\right) + {P_{N}}}}{{10}}}} = {\theta _{th}}
\end{split}
...
As for third and fourth throughput constraints, they are originally in form of generalized posynomials. However, the fifth and sixth delay constraints are not in form of generalized posynomials. But both of them are in form of the following inequality, which can be successfully transformed in the form of generalized posynomials:
...
Due to the various postures or some more strict QoS requirements, even the maximum transmission power, allocated by the above QoS optimization problem, cannot satisfy the QoS requirements. Considering the fact that reducing transmission rate can gain lower PLR with same SNR, we can carefully adjust the transmission rates for each nodes to assist the QoS optimization problem in responding to stricter QoS requirements. The discrete nature of optional transmission rates $\Re = \{ Rat{e_1},\cdots,Rat{e_h}\}$ makes the adjustment of the transmission rate difficult to be turned when the total throughput is limited.  
%Thus we allocate the transmission rates based on PLR requirement and link quality first before solving the optimization problem.
...
When PLR requirement is relax and can be satisfied through adjusting the transmission power, the maximum transmission rate is preferred, which ensures the throughput and delay requirements. However, when PLR requirement becomes stricter in several serious scenarios and cannot be met just through adjusting transmission power. For example, the patient go from bad to worse, the QoS requirements need to be correspondingly more strict. Therefore, the transmission rate allocation policy (TRAP) is proposed to obtain lower PLR through adjusting the transmission rate. 
...
According to the PLR requirement and link quality, the hub can get the transmission rate threshold ${R_{i,th}}$ for sensor $i$, which just satisfies the PLR constraint with the maximum transmission power $P_{i,tx,max}$.
